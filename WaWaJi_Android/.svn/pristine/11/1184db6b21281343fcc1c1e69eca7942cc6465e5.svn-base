...
public class PlayDetailsAct extends BaseActivity implements PlayDetailsContract.View {
    ...
    private PlayEntity playDetailsEntity;
    ...
    protected void setView() {
        ...
        //添加数据
        playDetailsEntity = (PlayEntity) bundle.getSerializable("entity");
        if (null == playDetailsEntity) {
            return;
        }
        ImageSetting.onImageSetting(this, playDetailsEntity.img, playImg);
        nameTv.setText(playDetailsEntity.title);
        timeTv.setText(playDetailsEntity.ctime);
        numberTv.setText(playDetailsEntity.number);
        serialNumberTv.setText(playDetailsEntity.clid);
        exchangeCurrencyTv.setText(playDetailsEntity.duihuan);
        nameUserPhone.setText(playDetailsEntity.name + " " + playDetailsEntity.phone);
        addressUser.setText(playDetailsEntity.address);
        orderUser.setText(playDetailsEntity.winnerid);
        nameUser.setText(playDetailsEntity.out_name);
        numberUser.setText(playDetailsEntity.out_id);
        switch (playDetailsEntity.status) {
            case "0":
                onGone();
     ...
     */
    @Override
    public void onBarrage(PlayEntity playDatailsEntity) {
