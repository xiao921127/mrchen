    ...
    private class CheckForLongPress extends WindowRunnnable implements Runnable {
        public void run() {
            ...
            if (child != null) {
                ...
                long longPressId = mAdapter.getItemId(mMotionPosition + mFirstPosition);
...
//        if (DBG) Log.e(TAG, "getSelectedView() is not supported in ExtendableListView yet");
     ...
     */
    private void addFooterView(View v, Object data, boolean isSelectable) {
...
//            case TOUCH_MODE_OVERSCROLL:
    ...
    private boolean onTouchUpScrolling(MotionEvent event) {
        if (hasChildren()) {
            ...
            boolean atEdge = mFirstPosition == 0 &&
...
//                boolean atEdge = false;
                if (incrementalDeltaY != 0) {
                    moveTheChildren(deltaY, incrementalDeltaY); // atEdge =
                }
...
//        if (DBG) Log.d(TAG, "moveTheChildren deltaY: " + deltaY + "incrementalDeltaY: " + incrementalDeltaY);
        ...
        int spaceBelow = getLastChildBottom() - end;
...
//            if (DBG) Log.d(TAG, "fillUp next - position:" + pos + " nextBottom:" + nextBottom);
    ...
    private void setupChild(View child, int position, int y, boolean flowDown,
        ...
        boolean isSelected = false; // TODO : selected && shouldShowSelector();
        boolean updateChildSelected = child.isSelected();
        int mode = mTouchMode;
        boolean isPressed = mode > TOUCH_MODE_DOWN && mode < TOUCH_MODE_SCROLLING &&
...
//        }
    ...
    private LayoutParams generateWrapperLayoutParams(View child) {
...
//            if (DBG) Log.d(TAG, "getView position:" + position);
     ...
     */
    protected void onLayoutChild(View child, int position,
                                 boolean flowDown, int childrenLeft, int childTop,
                                 int childRight, int childBottom) {
     ...
     */
    protected void onOffsetChild(View child, int position,
                                 boolean flowDown, int childrenLeft, int childTop) {
...
//        if (DBG) Log.d(TAG, "offsetChildrenTopAndBottom: " + offset);
        ...
        public void run() {
            switch (mTouchMode) {
                ...
                case TOUCH_MODE_FLINGING:
                    ...
                    boolean atEnd = moveTheChildren(delta, delta);
    ...
    class RecycleBin {
        ...
        public void markChildrenDirty() {
            if (mViewTypeCount == 1) {
                ArrayList<View> scrap = mCurrentScrap;
                ...
                for (int i = 0; i < typeCount; i++) {
                    ArrayList<View> scrap = mScrapViews[i];
                    int scrapCount = scrap.size();
...
//            if (DBG) Log.d(TAG, "addScrapView position = " + position);
        ...
        private void pruneScrapViews() {
            ...
            if (mTransientStateViews != null) {
                for (int i = 0; i < mTransientStateViews.size(); i++) {
                    View v = mTransientStateViews.valueAt(i);
    ...
    private static View retrieveFromScrap(ArrayList<View> scrapViews, int position) {
    ...
    private class PerformClick extends WindowRunnnable implements Runnable {
        ...
        public void run() {
            ...
            if (adapter != null && mItemCount > 0 &&
                ...
                View view = getChildAt(motionPosition); // a fix by @pboos

                if (view != null) {
                    int clickPosition = motionPosition + mFirstPosition;
    ...
    private boolean performLongPress(View child,
                                     int longPressPosition, long longPressId) {
        boolean handled = false;
